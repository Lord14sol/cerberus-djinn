...
export class DjinnClient {
    ...
    async fetchAllMarkets(): Promise<MarketData[]> {
        try {
            ...
            // Return empty array to avoid processing mocks when API is down
            return [];
        }
    }
    ...
    /**
     * Fetch markets expiring in the next N hours (for DRACO 12h resolution feed)
     */
    ...
    async sendVerificationResult(verdict: CerberusVerdict): Promise<boolean> {
        ...
        try {
            ...
            // NEW INTEGRATION: Send verification results to the Next.js API
            const updatePayload = {
                id: verdict.marketId,
            ...
            await axios.put(this.config.djinnApiUrl + '/markets', updatePayload, { headers });

            console.log(`[DJINN] Status ${verdict.finalStatus} synced to Next.js API for market ${verdict.marketId}`);
            return true;
        } catch (error) {
            console.log(`[DJINN] Error syncing result to Next.js API: ${error}`);
            return false;
        }
    }
